...
class MaintenanceTypeController : MaintenanceTypeControllerBase() {
    ...
    override fun onReadAttachment(session: HttpSession, request: HttpServletRequest, response: HttpServletResponse, mtid: String, name: String?): ByteArray? {
        //不提供此方法，返回404
        kotlin.run { response.status = HttpServletResponse.SC_NOT_FOUND; return null }
    }

    override fun onWriteAttachment(session: HttpSession, request: HttpServletRequest, response: HttpServletResponse, mtid: String, name: String?, bytes: ByteArray) {
        //不提供此方法，返回404
